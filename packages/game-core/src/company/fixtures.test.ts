...
export const PAYLOADS = {
  ...
  JoinFieldParty: {
    characterId: 'character-1',
    partyId: 'party-1',
    coLocationEvidenceId: 'meeting-1',
  },
  SetAssignment: {
    characterId: 'character-1',
    assignment: 'RECOVERY',
    locationId: 'home',
    dutyEvidenceId: 'handover-1',
    fundingPoolId: 'pool-1',
  },
  ...
  StartLearning: {
    characterId: 'character-1',
    methodId: 'book-study',
    goal: { workId: 'small-unit-service', sectionId: 'small-unit-service-1', maxTicks: '1000' },
    resourceIds: ['book-1'],
    budgetPoolId: 'pool-1',
    maxBudgetQ: '1000000',
  },
  ...
  CreditPractice: {
    ...
    challengeLevel: 5,
    outcome: 'MEANINGFUL_FAILURE',
    effortTicks: '1',
  },
  ChoosePerk: { characterId: 'character-1', perkId: 'blades-25-a', milestone: 25 },
  StartRetraining: {
    characterId: 'character-1',
    oldPerkId: 'blades-25-a',
    newPerkId: 'blades-25-b',
    mentorEvidenceId: 'mentor-1',
    budgetPoolId: 'pool-1',
  },
  ApplyCare: {
    characterId: 'character-1',
    ...
    resourceOrProviderReceiptId: 'care-1',
  },
  ApplyCondition: {
    receiptId: 'receipt-1',
    characterId: 'character-1',
  ...
  Observe: {
    ...
    observerRef: { kind: 'COMPANY', id: 'company-1' },
    subjectRef: { kind: 'CHARACTER', id: 'character-1' },
    factId: 'fact-1',
    sourceId: 'source-1',
  },
  Capture: {
    receiptId: 'receipt-1',
    characterId: 'character-1',
    captorRef: { kind: 'WORLD', id: 'bandits-1' },
    locationRef: place,
    seizedItems: [{ itemId: 'item-1', toContainerId: 'container-1', authorizationId: 'seizure-1' }],
  },
  ReleaseCaptive: {
    receiptId: 'receipt-1',
    characterId: 'character-1',
  ...
  TransferCaptive: {
    receiptId: 'receipt-1',
    characterId: 'character-1',
    fromCustodianId: 'captor-1',
    toCustodianId: 'captor-2',
    locationRef: place,
    exchangeProofId: 'exchange-1',
  },
  ResolveMissing: {
    resolutionId: 'resolution-1',
    characterId: 'character-1',
    notBefore: '1000',
    outcomeReceiptId: 'outcome-1',
  },
  RecordDeath: {
    receiptId: 'receipt-1',
    characterId: 'character-1',
    actualDeathTick: '900',
    causeId: 'cause-1',
    custodyOutcomeId: 'outcome-1',
  },
  ReturnToService: {
    characterId: 'character-1',
    arrivalEvidenceId: 'arrival-1',
    assignment: 'RECOVERY',
  },
  ...
  ClaimLoot: {
    ...
    toContainerId: 'container-1',
    accessEvidenceId: 'access-1',
    claimAuthorizationId: 'claim-1',
  },
  ApplyContainerLifecycle: {
    ...
    destinationId: 'container-2',
  },
  BeginEncounterBinding: {
    bindingId: 'binding-1',
    partyIds: ['party-1'],
    setupId: 'setup-1',
    bridgeVersion: 'm1-domain-bridge-v1',
    positionEvidenceId: 'position-1',
  },
...
export function example(type: CompanyCommandType) {
  const initial = {
    ...
    sourceEventId: 'source-1',
    expectedRevision: '0',
    actorRef: { kind: 'PLAYER', id: 'actor-1' },
    type,
...
describe('WP-02.1 finite command boundary', () => {
  ...
  for (const type of COMPANY_COMMAND_TYPES) {
    it(`${type}: exact shape, actor authority, no mutation or successful placeholder`, () => {
      ...
      expect(
        parseCompanyCommand({ ...value, payload: { ...value.payload, unexpected: true } }).ok,
      ).toBe(false);
  ...
  it('does not mistake declared receipt actors for authenticated authority', () => {
    ...
    const ctx = contextFor(value);
    expect(
      guardCompanyCommand(value, { ...ctx, principal: { kind: 'PLAYER', id: 'actor-1' } }).ok,
    ).toBe(false);
    const { internalGrant: _grant, ...withoutGrant } = ctx;
    expect(guardCompanyCommand(value, withoutGrant)).toEqual({
      ok: false,
      error: 'INVALID_SOURCE',
    });
    expect(
      guardCompanyCommand({ ...value, payload: { ...value.payload, actualDeathTick: '800' } }, ctx),
    ).toEqual({ ok: false, error: 'INVALID_SOURCE' });
    expect(
      parseCompanyCommand({ ...value, actorRef: { kind: 'SYSTEM_OR_PLAYER', id: 'actor-1' } }).ok,
    ).toBe(false);
  ...
  it('gates reason-dependent SYSTEM actions and permanent leadership choices', () => {
    ...
    const leadership = {
      ...example('ResolveLeadership'),
      actorRef: { kind: 'SYSTEM' as const, id: 'actor-1' },
    };
  ...
  it('freezes nested schema registries and rejects cross-discipline practice', () => {
    ...
    expect(
      parseCompanyCommand({
        ...example('CreditPractice'),
        payload: { ...PAYLOADS.CreditPractice, methodId: 'care-provided' },
      }).ok,
    ).toBe(false);
    expect(
      parseCompanyCommand({
        ...example('StartLearning'),
        payload: { ...PAYLOADS.StartLearning, methodId: 'funded-practice' },
      }).ok,
    ).toBe(false);
    expect(
      isJsonData(
  ...
  it('rejects disabled/unknown definitions, mismatched versions and references', () => {
    ...
    expect(
      parseCompanyCommand({ ...value, payload: { ...value.payload, skillId: 'alchemy-test' } }).ok,
    ).toBe(false);
    expect(
      parseCompanyCommand({ ...value, payload: { ...value.payload, methodId: 'missing' } }).ok,
    ).toBe(false);
    expect(parseCompanyCommand({ ...value, rulesetId: 's02-domain-provisional-0.1' }).ok).toBe(
      false,
    );
    ...
    expect(
      parseCompanyCommand({
        ...example('ChoosePerk'),
        payload: { characterId: 'character-1', perkId: 'blades-25-a', milestone: 60 },
      }).ok,
    ).toBe(false);
    expect(
      parseCompanyCommand({
        ...example('AdvanceCampaign'),
        payload: { toTick: '2000', authoritativeInputs: [{ actualStatus: 'DEAD' }] },
      }).ok,
    ).toBe(false);
  });
  it('rejects irreversible identity edits and malformed nested or extra fields', () => {
    ...
    expect(
      parseCompanyCommand({
        ...value,
        payload: { ...value.payload, appearancePatch: { birthName: 'replacement' } },
      }).ok,
    ).toBe(false);
    expect(
      parseCompanyCommand({
        ...example('CreateCompany'),
        payload: { ...PAYLOADS.CreateCompany, leaderInput: { ...identity, extra: true } },
      }).ok,
    ).toBe(false);
    expect(
      parseCompanyCommand({
        ...example('PayClaims'),
        payload: { ...PAYLOADS.PayClaims, mode: 'TARGETED' },
      }).ok,
    ).toBe(false);
    expect(
      parseCompanyCommand({
        ...example('PayClaims'),
        payload: { ...PAYLOADS.PayClaims, amountQ: 9007199254740992 },
      }).ok,
    ).toBe(false);
  });
});
...
describe('WP-02.1 exact values and safe JSON', () => {
  it('preserves money beyond safe-number precision and fails invalid encodings closed', () => {
    ...
    for (let i = 0; i < 1000; i += 1) {
      ...
      expect(
        JSON.parse(JSON.stringify({ tick: campaignTick(value), money: moneyQ(value) })),
      ).toEqual({ tick: value, money: value });
    }
  });
...
describe('WP-02.1 finite catalogue', () => {
  ...
  it('rejects invalid definitions, duplicate IDs, references and versions', () => {
    expect(validateCompanyCatalogue({ ...COMPANY_CATALOGUE, version: 'unknown' })).toContain(
      'UNSUPPORTED_CATALOGUE_VERSION',
    );
    expect(
      validateCompanyCatalogue({
        ...COMPANY_CATALOGUE,
        skills: [...COMPANY_CATALOGUE.skills, COMPANY_CATALOGUE.skills[0]],
      }),
    ).toContain('DUPLICATE_ID:skills');
    expect(
      validateCompanyCatalogue({
        ...COMPANY_CATALOGUE,
        species: [{ id: 'human', enabled: true, bodyId: 'missing', adultAtDays: 6570 }],
      }).some((e) => e.startsWith('BROKEN_REFERENCE')),
    ).toBe(true);
    expect(
      validateCompanyCatalogue({
        ...COMPANY_CATALOGUE,
        items: [{ ...COMPANY_CATALOGUE.items[0], weightG: -1 }],
      }),
    ).toEqual(['INVALID_CATALOGUE_SHAPE']);
    expect(
      validateCompanyCatalogue({
        ...COMPANY_CATALOGUE,
        perks: [{ ...COMPANY_CATALOGUE.perks[0], skillId: 'alchemy-test' }],
      }).some((e) => e.startsWith('BROKEN_REFERENCE')),
    ).toBe(true);
  });
});
