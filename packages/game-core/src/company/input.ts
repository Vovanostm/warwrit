...
type RequiredKeys<F extends Fields> = { [K in keyof F]: F[K] extends OptionalInput ? never : K }[keyof F];
...
export function isJsonData(value: unknown): value is JsonValue {
  ...
  function visit(item: unknown, depth: number): boolean {
    ...
    if (typeof item === 'number') return Number.isFinite(item) && !Object.is(item, -0) && (!Number.isInteger(item) || Number.isSafeInteger(item));
    ...
    if (Array.isArray(item) && (item.length > 1000 || keys.length !== item.length + 1)) return false;
...
export const text: Input<string> = { schema: { type: 'string', minLength: 1, maxLength: 4096 }, read: (v): v is string => typeof v === 'string' && v.length > 0 && v.length <= 4096 };
export const id: Input<string> = { schema: { type: 'string', minLength: 1, maxLength: MAX_ID_LENGTH }, read: isEntityId };
export const unsigned: Input<string> = { schema: { type: 'string', pattern: '^(0|[1-9][0-9]*)$', maxLength: MAX_EXACT_DIGITS }, read: (v): v is string => isExactInteger(v) };
export const signed: Input<string> = { schema: { type: 'string', pattern: '^(0|-?[1-9][0-9]*)$', maxLength: MAX_EXACT_DIGITS + 1 }, read: (v): v is string => isExactInteger(v, true) };
export const bool: Input<boolean> = { schema: { type: 'boolean' }, read: (v): v is boolean => typeof v === 'boolean' };
export function natural(minimum = 0, maximum = Number.MAX_SAFE_INTEGER): Input<number> {
  return { schema: { type: 'integer', minimum, maximum }, read: (v): v is number => typeof v === 'number' && Number.isSafeInteger(v) && !Object.is(v, -0) && v >= minimum && v <= maximum };
}
export function choice<const V extends readonly (string | number | boolean)[]>(...values: V): Input<V[number]> {
  return { schema: { enum: values }, read: (v): v is V[number] => values.some((value) => value === v) };
}
export function optional<T>(input: Input<T>): Input<T> & { readonly optional: true } {
  return { ...input, optional: true };
}
export function array<T>(input: Input<T>, minimum = 0, maximum = 1000, unique = false): Input<readonly T[]> {
  return { schema: { type: 'array', items: input.schema, minItems: minimum, maxItems: maximum, uniqueItems: unique }, read: (v): v is readonly T[] => Array.isArray(v) && v.length >= minimum && v.length <= maximum &&
    v.every((item: unknown) => input.read(item)) && (!unique || new Set(v).size === v.length) };
}
export function object<const F extends Fields>(fields: F): Input<ObjectOf<F>> {
  return { schema: { type: 'object', properties: Object.fromEntries(Object.entries(fields).map(([key, input]) => [key, input.schema])), required: Object.entries(fields).filter(([, input]) => !('optional' in input)).map(([key]) => key), additionalProperties: false }, read: (v): v is ObjectOf<F> => {
    if (!plainObject(v) || Object.keys(v).some((key) => !Object.hasOwn(fields, key))) return false;
...
export function either<A, B>(first: Input<A>, second: Input<B>): Input<A | B> {
  return { schema: { anyOf: [first.schema, second.schema] }, read: (v): v is A | B => first.read(v) || second.read(v) };
}
