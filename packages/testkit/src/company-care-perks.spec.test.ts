...
describe('B03 — CareRecovery/CareCost in actual care', () => {
  it('freezes the actual provider CareRecovery on material care and remains partition-invariant', () => {
    const beforeCare = injured(
      withMedicinePerk(economy([1n], 100n, 1000), 'medicine-25-a'),
      'severe-stable-wound',
    );
    ...
    expect(
      whole.physical!.conditions.find((entry) => entry.conditionId === target.conditionId)
        ?.resolvedAt,
    ).toBe('1228');

    const split = advance(advance(changedProvider, 1100).next, 1228).next;
    expect(
      split.physical!.conditions.find((entry) => entry.conditionId === target.conditionId)
        ?.resolvedAt,
    ).toBe('1228');
  });

  it('discounts only a supporting provider quote, rounds exact Q upward, and rejects insufficient funds atomically', () => {
    const quoted = injured(
      withMedicinePerk(economy([1n], 20n, 1000), 'medicine-25-b'),
      'severe-stable-wound',
    );
    ...
    expect(
      discounted.finance.wallets.find((entry) => entry.walletId === 'wallet-provider')?.cashQ,
    ).toBe('10');
  ...
  it('inherits one stabilization into the severe recovery without a second material charge', () => {
    ...
    expect(
      stabilized.physical!.conditions.find((entry) => entry.conditionId === target.conditionId)
        ?.resolvedAt,
    ).toBe('1000');
